...
tickers = c("UMGP", "WMG", "LYV", "TME", "SONY", "352820.KS", "041510.KQ")
...
LYV <- data.frame(LYV$LYV.Adjusted)
LYV <- LYV %>%
  rownames_to_column(var = "Date") %>%
  mutate(Date = as.Date(Date)) %>%
  rename(Price = LYV.Adjusted)

TME <- data.frame(TME$TME.Adjusted)
TME <- TME %>%
  rownames_to_column(var = "Date") %>%
  mutate(Date = as.Date(Date)) %>%
  rename(Price = TME.Adjusted)
...
stock_dataframes <- list(UMGP, WMG, LYV, TME, SONY, HYBE, SM)
stock_names <- list("UMGP", "WMG", "LYV", "TME", "SONY", "HYBE", "SM")
p <- plot_ly()
